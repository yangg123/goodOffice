...
- (NSString *)timeIntervalDescription
{
    ...
    if (timeInterval < 60) {
        ...
        return [NSString stringWithFormat:@"%d年前", (int)(timeInterval / 31536000)];
    }
}
...
- (NSString *)formattedDateDetailDescription
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    ...
    } else if (timeInterval < 86400) {//6小时内
        return [NSString stringWithFormat:@"%ld小时前", timeInterval / 3600];
    } else if ([[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]] == 86400) {//昨天
        [dateFormatter setDateFormat:@"HH:mm"];
        return [NSString stringWithFormat:@"1天前"];
    }else if ( [[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]] > 86400 && [[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]]<2592000) {//昨天
        long date = [[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]];
        double num = date/86400;
  
        return [NSString stringWithFormat:@"%d天前",(int)num];
    }
    else {//以前
        long date = [[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]];
        double num = date/2592000;
        
        return [NSString stringWithFormat:@"%d个月前",(int)num];
    }
}
...
+ (NSDate *)dateWithTimeIntervalInMilliSecondSince1970:(double)timeIntervalInMilliSecond {
    ...
    if(timeIntervalInMilliSecond > 140000000000) { //毫秒的话 / 1000
...
//    ret = [ret  dateByAddingTimeInterval: interval];
    
    return ret;
}


+ (NSString *)formattedDayFromTimeInterval:(long long)time{
    return [[NSDate dateWithTimeIntervalInMilliSecondSince1970:time] timeIntervalDescription];
}
...
+ (NSString *)formattedDateFromTimeInterval:(long long)time {
    
    return [[NSDate dateWithTimeIntervalInMilliSecondSince1970:time] formattedDateDescription];
}

+ (NSString *)formattedDetailDateFromTimeInterval:(long long)time{
      return [[NSDate dateWithTimeIntervalInMilliSecondSince1970:time] formattedDateDetailDescription];
}
